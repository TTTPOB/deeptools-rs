...
3. **Implement to match observed behavior.** Reference analysis confirmed Python's behavior is **region-level pre-filtering** (not signal-level masking):
   ...
   - **Empty group handling:** After filtering, error if all regions are removed. Per-group empty check only triggers under `--sortRegions keep` (matching Python's `computeMatrixOperations.sortMatrix()` path). For `no`/`ascend`/`descend`, empty groups are dropped from the header entirely (group_labels and group_boundaries omit them) and remaining task `group_index` values are remapped to contiguous indices.
