...
- 2025-10-30: Unified the streaming and in-memory pipelines around a shared row collector (`RowSink` + aggregator) so both modes now share the same scheduling loop; `reference_point` and `scale_regions` runners differ only in their per-mode row post-processing and header builders.
