...
- [ ] Advanced features (sorting, diagnostics polish) and performance tuning: pending.
    ...
    - 2025-10-30: streaming writer now spools rows to disk, emits a fixed-width stored gzip header member (rewritten in place once the final JSON is known), and runs on a dedicated `matrix-writer` thread; falls back to in-memory when the header would exceed the 4 KiB reserve or auxiliary sinks are requested. Observed today that `reference_point::run`/`scale_regions::run` still materialise the entire matrix before the writer thread begins, so streaming currently reduces peak heap but not end-to-end latency.
